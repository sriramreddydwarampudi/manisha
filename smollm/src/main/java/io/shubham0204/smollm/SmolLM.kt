...
class SmolLM {
    ...
    object DefaultInferenceParams {
        ...
        val chatTemplate: String =
            "{% for message in messages %}{% if loop.first and messages[0]['role'] != 'system' %}{{ '<|im_start|>system You are a friendly, caring AI companion named Amigo. Your goal is to support and chat with the user in a helpful, warm, and personal way. Remember details the user shares about themselves to provide a personalized experience. Keep your responses conversational and concise, like a friend on WhatsApp.<|im_end|> ' }}{% endif %}{{'<|im_start|>' + message['role'] + ' ' + message['content'] + '<|im_end|>' + ' '}}{% endfor %}{% if add_generation_prompt %}{{ '<|im_start|>assistant ' }}{% endif %}"
    }
